...
def get_season(date_str):
    ...
    try:
        ...
        if month in [12, 1, 2]:  # December, January, February
            ...
            return 'Summer'
        else:  # September, October, November (months 9, 10, 11)
            return 'Fall'
    except:
        return 'Unknown'
...
# The model will learn coefficients (weights) for each feature
model.fit(X_train, y_train)

# Make predictions on the test set
# Predictions are calculated as: y = β₀ + β₁x₁ + β₂x₂ + ... + βₙxₙ
...
with open('ModelWeights/linear_regressor.pkl', 'wb') as f:
    pickle.dump(model, f)
...
model_components = {
    ...
    'model_type': 'LinearRegression'         # Model type identifier
}

# Save model components and metadata using pickle for future inference
with open('ModelWeights/linear_regressor_components.pkl', 'wb') as f:
