...
# Linear regression will learn a coefficient for each binary feature created by one-hot encoding
model = LinearRegression()

# Train the model on the one-hot encoded training data
# The model learns: y = β₀ + β₁(Cat1_A) + β₂(Cat1_B) + β₃(Cat2_X) + β₄(Cat2_Y) + ... + ε
model.fit(X_train, y_train)

# Make predictions on the test set
# Each prediction is a linear combination of the binary features
